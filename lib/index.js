...
function ipGeoFilter(conf) {
  ...
  const continents = libFs.getJsonFile(path.join(__dirname, './continents.json'));
  // ---------------------------------
  // CONFIGURATION
  // ---------------------------------
  ...
  if (!conf.geoDBtype && conf.geoDB && typeof conf.geoDB === 'string' && conf.geoDB.lowercase().indexOf('.mmdb') > -1) {
    conf.geoDBtype = 'maxmind';
  } else if (!conf.geoDBtype && conf.geoDB && typeof conf.geoDB === 'string' && conf.geoDB.lowercase().indexOf('.bin') > -1) {
  ...
  return async function (ctx, next) {
    ...
    if (conf.development) {
      ...
      if (conf.context || ((!handled) && dbLoaded && (conf.allowCountry || conf.blockCountry || conf.allowContinent || conf.blockContinent))) {
        ...
        if (conf.geoDBtype === 'ip2location') {
          let data = ip2loc.IP2Location_get_all(ip);
          if (data) {
            const continent = data.country_short ? continents[data.country_short] : {
              continent_long: '-',
              continent_short: '-'
            }
            _city = data.city ? data.city : '-';
            _country = data.country_long ? data.country_long : '-';
            _continent = continent_long;
            _countryCode = data.country_short ? data.country_short : '-';
            _continentCode = continent_short;
